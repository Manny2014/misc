...
export const createArgoSubmitWftAction = (config: Config) => { 
  ...
  return createTemplateAction({
    ...
    async handler(ctx) {
        ...
        // Create client
        let client = new ArgoWorkflowServiceClient(config.get('argoWorkflows.server') as string, ns as string, ctx.logger);

        ctx.logger.info(`Running argo:wft:submit action template ${tplName} on namespace ${ns} with inputs ${inputs}`);
        
        try {
          const d = await client.submitWftAndWait(tplName as string, inputs as string[]);
